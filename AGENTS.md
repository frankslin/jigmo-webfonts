...
`npm run build` 會產生可直接部署的 `dist/`（Cloudflare Pages build output directory）。`split.py` 只複製三套 CSS 實際引用的 `fonts/*.woff2`，避免未引用字型混入部署產物。

`split_variants.py` 會解析每個 CSS 實際引用的 `fonts/*.woff2`，分別輸出 `dist-variants/jigmo/`、`dist-variants/jigmo-tc/`、`dist-variants/jigmo-sc/`。每個目錄只包含一套 CSS 與其實際用到的字型檔；加 `--with-site` 會把 `index.html` 和 `_headers` 也放進每個目錄。
...
python build.py --no-dl --no-shs   # 跳過 Source Han Serif

# 準備部署目錄
npm run build

# 分別輸出三套 CSS + 對應 fonts/（獨立網站或 npm package 用）
...
## Source Serif 4 / Source Han Serif 自托管
...
- Source Han Serif 常用 CJK 由本站自托管：原版與 TC CSS 使用 `shs-tc-*.woff2`，SC CSS 使用 `shs-sc-*.woff2`。兩者都只切 CJK punctuation + URO。`shs-jp-*.woff2` 可由 build 腳本下載/生成，但不要塞進這三套 CSS 產物。
- SS4 / Source Han woff2 都部署到單一 `dist/`。
...
| 範圍 | 授權 |
|------|------|
| 本專案原始碼與網頁（build.py、split.py、index.html 等） | MIT |
| Jigmo 字型（神地康一） | CC0 1.0（公共領域） |
| Source Serif 4 字型（Adobe） | OFL-1.1 |

詳見 `LICENSE`（MIT）與 `THIRD_PARTY_LICENSES.md`（第三方授權全文）。
...
建議 CSS `font-family` 排列順序（Source Han 先覆蓋常用字，Jigmo 補 Ext-B+）：

```css
font-family: 'Source Serif 4', 'Source Han Serif TC', 'Jigmo TC', serif;
```
